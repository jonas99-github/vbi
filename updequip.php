...
<span>ID </span>
<input type="text" name="id" value="<?php echo $row['eq_inv_id'];?>" style="background-color:#ccc;" readonly><br><br>
...
<select name="brand" required>
  ...
  while($row4=mysqli_fetch_assoc($result4)){
    ?>
  <option><?php echo $row4['brand'];?></option>
<?php
}
$sql3="SELECT * FROM brand ORDER BY brand";
$result3=mysqli_query($conn,$sql3);
while($row3=mysqli_fetch_assoc($result3)){
  ?>
  <option><?php echo $row3['brand'];?></option>
  <?php
}
 ?>
</select>
...
<span>Price: </span>
<input type="number" name="price" value="<?php echo $row['price'];?>" style="height:30px; width:267px;">
<span>Location: </span>
